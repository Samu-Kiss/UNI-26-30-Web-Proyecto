...
begin
    if not exists (select 1 from pg_constraint
            where conname = 'ck_estudiantes_semestre' and conrelid = 'app.estudiantes'::regclass) then
        alter table app.estudiantes add constraint ck_estudiantes_semestre
            check (semestre between 1 and 20);
    end if;
    if not exists (select 1 from pg_constraint
            where conname = 'ck_tutores_tarifa_positiva' and conrelid = 'app.tutores'::regclass) then
        alter table app.tutores add constraint ck_tutores_tarifa_positiva
            check (tarifa_por_hora > 0);
    end if;
    if not exists (select 1 from pg_constraint
            where conname = 'ck_tutores_calificacion' and conrelid = 'app.tutores'::regclass) then
        alter table app.tutores add constraint ck_tutores_calificacion
            check (calificacion_promedio between 0 and 5);
    end if;
    if not exists (select 1 from pg_constraint
            where conname = 'ck_reservas_duracion_positiva' and conrelid = 'app.reservas'::regclass) then
        alter table app.reservas add constraint ck_reservas_duracion_positiva
            check (duracion_minutos > 0);
    end if;
    if not exists (select 1 from pg_constraint
            where conname = 'ck_reservas_costo_positivo' and conrelid = 'app.reservas'::regclass) then
        alter table app.reservas add constraint ck_reservas_costo_positivo
            check (costo_total > 0);
    end if;
    if not exists (select 1 from pg_constraint
            where conname = 'ck_reservas_estado' and conrelid = 'app.reservas'::regclass) then
